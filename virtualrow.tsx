...
import { KfIcon, Drag, ICON_SIZE, ICON_STROKE } from "kf-icons";
  ...
  return (
    <Fragment key={row.id}>
      <tr
        ...
        style={style}
        className={`tableRow flex absolute w-full cursor-pointer ${isAccordionExpandable || isLastSubRow ? "border-b-1 border-solid border-gray-200" : ""} flex-nowrap ${
  ...
  return (
    <Fragment key={row.id}>
      <tr
        ref={setNodeRef}
        style={style}
        className={`tableRow flex relative w-full cursor-pointer ${isLastSubRow ? "border-b-1 border-solid border-gray-200" : ""} flex-nowrap ${row.getIsSelected() ? "bg-primary-25" : "bg-white"} hover:bg-gray-50 group/tablerow`}
        onClick={handleRowClick}
        onKeyDown={handleRowClick}
      >
        {row.parentId &&
...
export const RowDragHandleCell = ({ rowId }: { rowId: string }) => {
  ...
  return (
    ...
    >
      <KfIcon name={Drag} size={ICON_SIZE.SIZE_16} stroke={ICON_STROKE.THIN} />
    </div>
  );
};
