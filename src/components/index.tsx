...
import { tableVariants, tableContainerVariants } from "../variants/table";
...
import { SerialNumberColumn } from "./serial_number";
import { defaultColumn } from "../utils";
import { TableHeader } from "./header";
import { TableBody } from "./body";
import { TableProvider } from "./tablecontext";
import { DEFAULT_KEYS, EXPANDABLE_TYPES, UPDATED_EVENTS } from "../constant";
...
export interface TableProps {
  ...
  /**
   * Slot-based rendering for custom components
   */
  slots?: {
  ...
  /**
   * Applies striped styling to table rows.
   */
  striped?: boolean;

  /**
   * Applies bordered styling to the table and cells.
   */
  bordered?: boolean;

  /**
   * Applies compact styling to the table, reducing padding.
   */
  compact?: boolean;
  ...
  /**
   * Sets the density of the table (e.g., 'comfortable', 'compact', 'spacious').
   */
  ...
  /**
   * Theme for the table (e.g., 'light', 'dark').
   */
  theme?: 'light' | 'dark';

  /**
   * Custom class names for various table elements.
   */
  classNames?: {
    table?: string;
  ...
  /**
   * Custom component for server-side pagination.
   */
  Pagination?: React.ComponentType<any>;

  /**
   * Custom component for the table toolbar.
   */
  Toolbar?: React.ComponentType<any>;

  /**
   * Custom component for export buttons.
   */
  ExportButtons?: React.ComponentType<any>;
}
...
export const DataTable = forwardRef<DataTableRef, TableProps>((props, ref) => {
  ...
  const [currentGrouping, setCurrentGrouping] = useState<string[]>(grouping.enabled ? grouping.groupBy : []);
  ...
  return (
    ...
    >
      <div
        className={cn("butterfly-components", className)}
        style={{
          display: "grid",
          height: "inherit"
        }}
      ...
      >
        {Toolbar && <Toolbar />}
        <div
          className={cn(tableContainerVariants({ bordered }), classNames?.tableContainer)}
          ref={tableContainerRef}
          onScroll={onScroll}
          style={{ maxHeight: `${tableHeight}px` }}
        >
          <table className={cn(tableVariants({ striped, bordered, compact, hoverable, density }), classNames?.table)}>
            {slots?.header ? <slots.header /> : <TableHeader />}
            {data.length === 0 ? (
              slots?.emptyState ? (
                <slots.emptyState emptyState={emptyState} />
              ) : (
